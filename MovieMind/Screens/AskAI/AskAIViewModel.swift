...
final class AskAIViewModel {
    ...
    struct Message: Identifiable {
        ...
        /// Locally generated text; never replayed to the model.
        var isNotice = false
    }

    /// Gemini is stateless, so history is re-sent each turn — but only a window of it.
    ...
    init(chatService: AIChatServicing = AIChatService.shared) {
    ...
    private var conversationHistory: [AIChatTurn] {
        ...
        // Gemini expects the exchange to begin with a user turn.
